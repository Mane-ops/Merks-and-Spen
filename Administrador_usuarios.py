...
title_label.grid(row=0, column=0, columnspan=9, pady=(0, 10))

# Crear el subtítulo
subtitle_label = ttk.Label(main_frame, text="Administrador de Usuarios", style='TLabel', font=('Arial', 14))
subtitle_label.grid(row=1, column=0, columnspan=9, pady=(0, 20))

# Crear la tabla de 9x5
for i in range(5):  # Filas
    for j in range(9):  # Columnas
        if i == 0:
            ttk.Label(main_frame, text=["Id", "Nombre", "Apellido", "Rol", "Correo", "Contraseña", "Editar", "Eliminar", "Guardar"][j],
...
for i, image in enumerate([edit_image]):
    for j in range(6, 9):  # Columnas correspondientes a las imágenes
...
for i, image in enumerate([delete_image]):
    for j in range(7, 9):  # Columnas correspondientes a las imágenes
        for row in range(3, 7):  # Filas correspondientes a las imágenes
            canvas = tk.Canvas(main_frame, width=20, height=20)
            canvas.create_image(10, 10, anchor=tk.CENTER, image=image)
            canvas.grid(row=row, column=j, padx=5, pady=5)
...
for i, image in enumerate([save_image]):
    for j in range(8, 9):  # Columnas correspondientes a las imágenes
        for row in range(3, 7):  # Filas correspondientes a las imágenes
            canvas = tk.Canvas(main_frame, width=20, height=20)
            canvas.create_image(10, 10, anchor=tk.CENTER, image=image)
            canvas.grid(row=row, column=j, padx=5, pady=5)
